...
// ---- Carga bajo demanda de librerías para exportar PDF (desde cdnjs) ----
...
// ---- Exporta uno o más elementos del DOM (por id) a un PDF con paginación ----
async function renderElementsToPdf(ids, filename) {
  await ensurePdfLibs();
  const { jsPDF } = window.jspdf;
  const pdf = new jsPDF("p", "pt", "a4");
  const pageW = pdf.internal.pageSize.getWidth();
  const pageH = pdf.internal.pageSize.getHeight();
  let firstDoc = true, any = false;
  for (const id of ids) {
    ...
    firstDoc = false; any = true;
  }
  if (!any) throw new Error("Sin contenido para exportar");
  pdf.save(filename);
}
...
function WorkoutRoutineChooser({ userProfile }) {
  ...
  const [loading, setLoading] = useState(false);
  const [wPdfLoading, setWPdfLoading] = useState(false);
  const [wPdfError, setWPdfError] = useState("");

  const types = ["🏋️ Gimnasio", "🏃 Cardio", "🧘 Pilates", "🏠 En casa", "⚡ HIIT", "🚴 Ciclismo"];

  const downloadRoutine = async () => {
    setWPdfError(""); setWPdfLoading(true);
    try { await renderElementsToPdf(["fudnfuel-workout"], `FudnFuel-rutina-${new Date().toISOString().slice(0, 10)}.pdf`); }
    catch { setWPdfError("No se pudo generar el PDF. Intenta de nuevo."); }
    setWPdfLoading(false);
  };
  ...
  return (
    <div style={{ ...cardStyle, borderLeft: `4px solid ${C.accent}` }}>
      ...
      {routine && (
        <div>
          <div id="fudnfuel-workout" style={{ background: "#fff", borderRadius: 12, padding: "18px 16px" }}>
            <div style={{ textAlign: "center", marginBottom: 14, paddingBottom: 12, borderBottom: `2px solid ${C.light}` }}>
              <div style={{ fontSize: 11, letterSpacing: 2, textTransform: "uppercase", color: C.accent, fontWeight: 700 }}>FudnFuel</div>
              <div style={{ fontSize: 19, fontWeight: 800, color: C.primary, marginTop: 2 }}>Tu rutina de entrenamiento</div>
              <div style={{ fontSize: 12, color: C.muted, marginTop: 3 }}>
                {type ? type.replace(/[^0-9A-Za-zÁÉÍÓÚáéíóúÑñ ]/g, "").trim() : ""} · Meta: {userProfile?.goal}
              </div>
            </div>
            <MarkdownTable text={routine} />
          </div>
          <div style={{ marginTop: 12, textAlign: "center" }}>
            <button onClick={downloadRoutine} disabled={wPdfLoading}
              style={{ ...btnPrimary, opacity: wPdfLoading ? 0.7 : 1, cursor: wPdfLoading ? "default" : "pointer" }}>
              {wPdfLoading ? "⏳ Generando PDF..." : "⬇️ Descargar rutina PDF"}
            </button>
            {wPdfError && <div style={{ color: "#c0392b", fontSize: 13, marginTop: 8, fontWeight: 600 }}>⚠️ {wPdfError}</div>}
          </div>
        </div>
      )}
    </div>
...
function parseAndRenderPlan(planData) {
  const { raw, isFemale, phaseInfo, userProfile } = planData;
  const sections = [];
  ...
  return sections.map((sec, idx) => {
    ...
    if (t.includes("análisis") || t.includes("analisis") || t.includes("cuerpo")) return (
    ...
    if (t.includes("suplemento") || t.includes("vitamina")) return (
      <div key={idx} style={{ ...cardStyle, borderLeft: `4px solid ${C.yellow}` }}>
        <div style={{ fontWeight: 700, fontSize: 17, color: C.primary, marginBottom: 12 }}>{sec.title}</div>
        <div style={{ fontSize: 14, color: C.muted, marginBottom: 10 }}>Lo que recomendamos:</div>
    ...
    if (t.includes("glucosa") || t.includes("pico") || t.includes("azúcar")) return (
    ...
    if (t.includes("dato") || t.includes("curiosidad") || t.includes("sabías") || t.includes("lección")) {
...
// ---- Extrae la sección de ciclo del plan y arma las 4 fases para el PDF ----
function extractCycleBody(raw) {
  if (!raw) return "";
  const m = raw.match(/##\s+[^\n]*?(?:Ciclo|Femenin|Cycle)[\s\S]*?(?=\n##\s+|$)/i);
  return m ? m[0].replace(/##[^\n]*\n/, "") : "";
}

function CycleAllPhasesPrint({ body }) {
  const defs = [
    ["Menstrual", "Días 1–5", "#e8a0b0", "#fdf0f3"],
    ["Folicular", "Días 6–13", "#a8d8a8", "#eef3e6"],
    ["Ovulatoria", "Días 14–16", "#f9c784", "#f7f0df"],
    ["Lútea", "Días 17–28", "#b0b8e8", "#eeeef6"],
  ];
  const blocks = {};
  defs.forEach(([p]) => {
    const re = new RegExp(`###\\s+(?:Fase\\s+)?${p}[\\s\\S]*?(?=###\\s+|$)`, "i");
    const m = body && body.match(re);
    if (m) blocks[p] = m[0].replace(/###.+\n/, "").trim();
  });

  return (
    <div>
      <div style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: C.accent, fontWeight: 700, textAlign: "center" }}>FudnFuel · Cycle Syncing</div>
      <div style={{ fontSize: 21, fontWeight: 800, color: C.primary, textAlign: "center", marginTop: 2 }}>Guía por fase de tu ciclo</div>
      <div style={{ fontSize: 11, color: C.muted, textAlign: "center", margin: "8px auto 20px", maxWidth: 620, lineHeight: 1.6 }}>
        Las fases asumen un ciclo de ~28 días; ajusta los días a tu propio patrón. Combinamos recomendaciones con respaldo firme (hierro en la menstruación, magnesio y B6 para el SPM) con guías flexibles de evidencia todavía limitada.
      </div>
      {defs.map(([p, days, color, soft]) => (
        <div key={p} style={{ background: soft, borderRadius: 14, padding: "14px 18px", marginBottom: 12, borderLeft: `4px solid ${color}` }}>
          <div style={{ fontWeight: 800, color: C.primary, fontSize: 16 }}>
            Fase {p} <span style={{ fontWeight: 600, color: C.muted, fontSize: 12 }}>· {days}</span>
          </div>
          <div style={{ fontSize: 13, lineHeight: 1.75, marginTop: 6, color: C.text }}>
            {blocks[p] ? renderText(blocks[p]) : "Información de fase no disponible."}
          </div>
        </div>
      ))}
      <div style={{ fontSize: 10.5, color: C.muted, textAlign: "center", marginTop: 10, lineHeight: 1.5 }}>
        Documento educativo y de organización personal. Acompaña tu tratamiento médico; cualquier ajuste o suplemento se revisa con tu profesional de salud.
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const exportPDF = async () => {
    ...
    try {
      const ids = ["fudnfuel-plan"];
      if (planData?.isFemale && includeCycle) ids.push("fudnfuel-cycle-print");
      await renderElementsToPdf(ids, `FudnFuel-plan-${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (e) {
      setPdfError("No se pudo generar el PDF. Revisa tu conexión e intenta de nuevo.");
    }
    setPdfLoading(false);
  };

  // Para mujeres con cycle syncing activo, el plan se descarga una vez que
  // registran la fecha de su periodo y la duración de su ciclo (phaseInfo).
  const needsPeriodForPdf = !!(planData && planData.isFemale && includeCycle && !phaseInfo);
...
- Peso: ${form.weight} ${form.weightUnit} | Altura: ${form.height} ${form.heightUnit}
- Meta: ${form.goal}
...
Lista 5-6 nombres de suplementos/vitaminas (uno por línea, como punto de lista). Solo el nombre, sin descripción.

## 🩸 Cómo Evitar Picos de Glucosa
Lista 6 consejos prácticos como puntos de lista. Cada consejo de 1-2 oraciones.

## 💡 Datos Curiosos y Micro-Lecciones
Lista exactamente 6 datos curiosos y científicos sobre nutrición o movimiento relevantes para su meta. Uno por punto de lista.

${isFemale ? `## 🌸 Guía del Ciclo Femenino
Proporciona orientación específica para cada fase. Usa ### para cada nombre de fase exactamente como está escrito. En "Mejores alimentos" y "Suplementos", cuando una recomendación tenga respaldo científico firme agrega "(evidencia sólida)" y cuando sea una práctica popular con datos limitados agrega "(evidencia limitada)".
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Segoe UI', system-ui, sans-serif", padding: "24px 16px" }}>
      <div style={{ maxWidth: 660, margin: "0 auto" }}>
        ...
        {planData && (
          <div>
            {/* Controles (fuera de la captura del PDF) */}
            <div style={{ display: "flex", gap: 10, justifyContent: "center", alignItems: "center", flexWrap: "wrap", marginBottom: 8 }}>
              <button onClick={exportPDF} disabled={pdfLoading || needsPeriodForPdf}
                title={needsPeriodForPdf ? "Agrega la fecha de tu periodo y la duración de tu ciclo para descargar" : ""}
                style={{ ...btnPrimary, opacity: (pdfLoading || needsPeriodForPdf) ? 0.55 : 1, cursor: (pdfLoading || needsPeriodForPdf) ? "default" : "pointer" }}>
                {pdfLoading ? "⏳ Generando PDF..." : "⬇️ Descargar plan PDF"}
              </button>
              {planData.isFemale && (
                <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13.5, fontWeight: 600, color: C.primary, background: "#fff", border: `1.5px solid #b8ccaa`, borderRadius: 12, padding: "9px 14px", cursor: "pointer" }}>
                  ...
                  Incluir cycle syncing (todas las fases)
                </label>
              )}
            </div>
            {planData.isFemale && (
              <p style={{ textAlign: "center", fontSize: 12, color: needsPeriodForPdf ? "#b5732b" : C.muted, marginBottom: 16 }}>
                {needsPeriodForPdf
                  ? "🗓️ Para descargar tu plan con cycle syncing, primero registra la fecha de tu periodo y la duración de tu ciclo aquí abajo."
                  : "Tu PDF agrega la guía por fase del ciclo al final. Desmarca la casilla para descargar solo el plan."}
              </p>
            )}
            {pdfError && <div style={{ color: "#c0392b", fontSize: 13, textAlign: "center", marginBottom: 12, fontWeight: 600 }}>⚠️ {pdfError}</div>}

            {/* Región capturada en el PDF */}
            <div id="fudnfuel-plan" style={{ background: "#fff", borderRadius: 18, padding: "26px 24px" }}>
              <div style={{ textAlign: "center", marginBottom: 20, paddingBottom: 16, borderBottom: `2px solid ${C.light}` }}>
                <div style={{ fontSize: 12, letterSpacing: 2, textTransform: "uppercase", color: C.accent, fontWeight: 700 }}>FudnFuel</div>
                <div style={{ fontSize: 23, fontWeight: 800, color: C.primary, marginTop: 2 }}>Tu plan personalizado</div>
                <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>Meta: {planData.userProfile?.goal} · {new Date().toLocaleDateString("es-MX")}</div>
              </div>
              ...
              <div style={{ ...cardStyle, background: C.primary, textAlign: "center", marginBottom: 0 }}>
            ...
            {/* Apéndice de ciclo (fuera de pantalla) que se agrega al PDF cuando la casilla está activa */}
            {planData.isFemale && includeCycle && (
              <div id="fudnfuel-cycle-print" aria-hidden="true"
                style={{ position: "absolute", left: -99999, top: 0, width: 760, background: "#fff", padding: "30px 28px", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
                <CycleAllPhasesPrint body={extractCycleBody(planData.raw)} />
              </div>
            )}

            <button style={{ ...btnPrimary, width: "100%", borderRadius: 14, padding: 14, fontSize: 15, margin: "16px 0 32px" }}
              onClick={() => { setPlanData(null); setPhaseInfo(null); setPdfError(""); setForm({ age:"",weight:"",weightUnit:"kg",height:"",heightUnit:"cm",sex:"",goal:"",diet:"",exercise:"",water:"",allergies:"",conditions:"" }); }}>
              🔄 Empezar de nuevo
            </button>
          </div>
        )}
